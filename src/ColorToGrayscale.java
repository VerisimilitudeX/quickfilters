...
public class ColorToGrayscale {
    ...
    for (Pixel p : grayImage.pixels()) {
      ...
      if (algorithm == 5) {
        ...
        int avg =
            (int)
                Math.round(
                    (red * corrPixel.getRed())
                        + (green * corrPixel.getGreen())
                        + (blue * corrPixel.getBlue()));
        p.setRed(avg);
        p.setGreen(avg);
        p.setBlue(avg);
        sc.close();
  ...
  public static void main(String[] args) {
    ...
    System.out.println(
        "CONVERSION COMPLETE: The grayscale images are saved in the same directory as the original"
            + " images, with a -gray suffix.");
  }
}
